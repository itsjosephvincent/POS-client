...
const rowStyle = `flex justify-start items-center gap-4 px-4 py-2 pl-4 cursor-pointer mx-4 my-2 rounded-xl`
        ...
        <div class="w-full md:flex md:flex-col md:items-start">
            <NuxtLink class="w-full" v-for="item in props.menus" :to="item.link" :key="item.id">
                <div id="row"
                    ...
                    <IconSvg :color="currentPage === item.name ? 'var(--secondary-color)' : 'var(--text-primary)'" :key="item.id" :icon="item.icon" size="1em" />
                    <div :class="[labelStyle, currentPage === item.name ? 'text-secondaryColor' : '']">{{ item.name }}</div>
                </div>
            </NuxtLink>
        </div>
