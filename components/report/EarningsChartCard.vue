...
onMounted(() => {
    ...
    const drawLines = (item: object, strokeColor: string, fillColor: string) => {
        svg
            .append('path')
            .datum(item.data)
            .attr('fill', 'none')
        ...
        svg
            ...
            .on('mouseover', (event: any, d: any) => {
                tooltip.style('visibility', 'visible').text(`${item.name} Revenue: ${d.value}`)
            })
    ...
    <div class="bg-secondaryBg p-4 rounded-xl border border-primaryBorder text-primaryText">
        ...
        <div class="flex justify-end items-center my-1">
            <div v-for="(item, index) in data" class="flex justify-start items-center gap-1 mx-2">
                <span :class="['p-2 rounded-md border border-primaryBorder']" :style="`background: ${colors[index]}`"></span>
