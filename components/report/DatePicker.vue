...
function onDateChange() {
    if (selectedDate.value) {
        formattedDate.value = new Date(selectedDate.value).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
        console.log(selectedDate.value)
        console.log(formattedDate.value)
    }
}
const dateLabel = computed(() => !formattedDate.value ? 'Select Date' : `${formattedDate.value}`)
        ...
        <div class="flex justify-start items-center">
            <div class="pb-4">
                ...
                <input
                    ...
                    type="date" id="datepicker" v-model="selectedDate" @input="onDateChange" />
            </div>
        </div>
    </div>
</template>