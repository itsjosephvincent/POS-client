...
<template>
    <div class="w-full h-[calc(100vh-304px)] overflow-y-auto">
        <div v-for="item in billedItems" class="px-4 py-2 flex justify-between items-center text-primaryText gap-2">
            ...
            <div class="">
                <div class="font-medium text-secondaryText text-xs text-right">{{ item.quantity }} x {{ item.price }}</div>
                <div class="font-bold text-right">{{ (item.price * item.quantity).toFixed(2) }}</div>
            </div>
        </div>
    </div>
</template>