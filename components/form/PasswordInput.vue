...
<template>
    <div class="w-full">
        <div :class="['relative h-12 rounded-xl flex items-center bg-primaryBg border border-primaryBorder',]">
            <IconSvg v-if="props.icon" class="mr-4 absolute left-[10px]" icon="password" color="tertiaryText" size="2em" />
            ...
            <IconSvg @click="toggleMode" class="mr-4 absolute right-[10px]" :icon="currentIcon" color="tertiaryText" size="2em" />
        </div>
    </div>
</template>
