    ...
    <div class="w-full lg:w-[500px] p-6 bg-secondaryBg lg:border lg:border-primaryBorder lg:rounded-xl flex flex-col md:flex md:flex-row justify-center items-center gap-4 cursor-pointer text-primaryColor">
        ...
        <div class="w-full flex flex-col items-start grow">
            ...
            <div class="font-bold text-primaryText">
                <span class="pr-2 font-medium text-secondaryText">Cost</span>{{ props.cost }}
            </div>
            <div class="font-bold text-primaryText">
            ...
            <div class="w-full flex flex-col md:flex-row gap-2 mt-4">
                ...
                <PrimaryButton :loading="getDeleteLoading" @click="clickDelete" label="Delete" custom-class="bg-errorColor text-white" class="w-full h-[38px]"/>
            </div>

            <Teleport to="body">
                <ModalDelete :visible="getShowDeleteModal" @close-delete-modal="closeDeleteModal" @delete-confirmed="handleDelete" >
                    <div class="flex items-center gap-4 my-4">
                        <IconSvg icon="error" color="errorColor" />
                        <div class="text text-primaryText">Are you sure you want to delete <span class="font-bold">{{ props.name || 'this product' }}</span>?</div>
                    </div>
                </ModalDelete>
            </Teleport>
        </div>
